    ...
    if($modeaplikasi == "Demo") {
        ...
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
            if($ukuran < 1044070){			
                ...
                $stmt = $mysqli->prepare("INSERT INTO potensidesa (judul, isi, foto) VALUES (?, ?, ?)");
...
if(isset($_POST['simpan'])) {
    if($modeaplikasi == "Demo") {
        ...
        if($_FILES['foto']['name'] == "") {
            $stmt = $mysqli->prepare("UPDATE potensidesa SET judul = ?, isi = ? WHERE id = ?");
            ...
            $stmtv2 = $mysqli->prepare("SELECT * FROM potensidesa WHERE id = ?");
                ...
                if(isset($foto) && unlink("$foto")) {
                    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
                        if($ukuran < 1044070){			
                            ...
                            $stmt = $mysqli->prepare("UPDATE potensidesa SET judul = ?, isi = ?, foto = ? WHERE id = ?");
...
if(isset($_POST['hapus'])) {
    if($modeaplikasi == "Demo") {
        ...
        if($resultv2->num_rows === 0) {} else {
            ...
            if(isset($foto) && unlink("$foto")) {
                $stmt = $mysqli->prepare("DELETE FROM potensidesa WHERE id = ?");
...
<html lang="en">
    <head>
        <title><?php echo $rowpengaturanweb['namaweb'];?> &mdash; Potensi Desa</title>
        ...
        <div id="app">
            <div class="main-wrapper main-wrapper-1">
                ...
                <div class="main-content">
                                                ...
                                                <table id="datatable" class="table display nowrap" width="100%" style="overflow-x: hidden !important;">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th>Judul Potensi Desa</th>
                                                            <th>Aksi</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                    <?php
                                                    $stmt = $mysqli->prepare("SELECT * FROM potensidesa");
                                                    $stmt->execute();
                    ...
                    <?php
                    $stmtv321 = $mysqli->prepare("SELECT * FROM potensidesa");
                    $stmtv321->execute();
                            ...
                            <div class="modal-content">
                                ...
                                <form action="" method="post" enctype="multipart/form-data" autocomplete="off">
                                    <div class="modal-body mb-0">
                                        ...
                                        <div class="form-group">
                                            <label>Judul Potensi Desa</label>
                                            <input type="text" class="form-control" placeholder="Masukkan judul potensi desa..."  name="judul" required>
                                        </div>
                                        <div class="form-group">
                                            <label>Deskripsi Potensi Desa</label>
                                            <textarea class="summernote" name="subjudul" placeholder="Masukkan deskripsi potensi desa..." required></textarea>
                                        </div>
                                    </div>
                                    <div class="modal-footer pt-0 mt-0">
