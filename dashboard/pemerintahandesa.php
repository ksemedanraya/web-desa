    ...
    if($modeaplikasi == "Demo") {
        ...
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
            if($ukuran < 1044070){			
                ...
                $stmt = $mysqli->prepare("INSERT INTO pemerintahdesa (nama, jabatan, foto) VALUES (?, ?, ?)");
...
if(isset($_POST['simpan'])) {
    if($modeaplikasi == "Demo") {
        ...
        if($_FILES['foto']['name'] == "") {
            $stmt = $mysqli->prepare("UPDATE pemerintahdesa SET nama = ?, jabatan = ? WHERE id = ?");
            ...
            $stmtv2 = $mysqli->prepare("SELECT * FROM pemerintahdesa WHERE id = ?");
                ...
                if(isset($foto) && unlink("$foto")) {
                    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
                        if($ukuran < 1044070){			
                            ...
                            $stmt = $mysqli->prepare("UPDATE pemerintahdesa SET nama = ?, jabatan = ?, foto = ? WHERE id = ?");
...
if(isset($_POST['hapus'])) {
    if($modeaplikasi == "Demo") {
        ...
        if($resultv2->num_rows === 0) {} else {
            ...
            if(isset($foto) && unlink("$foto")) {
                $stmt = $mysqli->prepare("DELETE FROM pemerintahdesa WHERE id = ?");
...
<html lang="en">
    <head>
        <title><?php echo $rowpengaturanweb['namaweb'];?> &mdash; Pemerintahan Desa</title>
                ...
                <div class="main-content">
                    <section class="section">
                        <div class="section-header">
                            <h1>Pemerintahan Desa (Struktur Organisasi)</h1>
                                                ...
                                                <table id="datatable" class="table display nowrap" width="100%" style="overflow-x: hidden !important;">
                                                    ...
                                                    if($result->num_rows === 0) {} else { while($row = $result->fetch_assoc()) {?>
                                                        <tr>
                                                            ...
                                                            <td class="pt-3"><?php echo $row['jabatan'];?></td>
                                                            <td>
                    ...
                    <?php
                    $stmtv321 = $mysqli->prepare("SELECT * FROM pemerintahdesa");
                    $stmtv321->execute();
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-12">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control"  name="judul" required value="<?php echo $row321['nama'];?>">
                                                    </div>
                                                </div>
                                                <div class="col-12">
                                                    <div class="form-group">
                                                        <label>Jabatan</label>
                                                        <input type="text" class="form-control"  name="subjudul" required value="<?php echo $row321['jabatan'];?>">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <form action="" method="post" enctype="multipart/form-data" autocomplete="off">
                                    <div class="modal-body mb-0">
                                        <div class="form-group">
                                            ...
                                            <p class="mb-0 text-danger" style="font-size: 11px;">Untuk hasil terbaik, siapkan foto dengan ukuran: lebar 300 piksel & tinggi 300 piksel</p>
                                        </div>
                                        <div class="form-group">
                                            <label>Nama Lengkap</label>
                                            <input type="text" class="form-control" placeholder="Masukkan nama lengkap..."  name="judul" required>
                                        </div>
                                        <div class="form-group">
                                            <label>Jabatan</label>
                                            <input type="text" class="form-control" placeholder="Masukkan jabatan..."  name="subjudul" required>
                                        </div>
                                    </div>
                                    <div class="modal-footer pt-0 mt-0">
