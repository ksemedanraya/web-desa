    ...
    if($modeaplikasi == "Demo") {
        ...
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
            if($ukuran < 1044070){			
                ...
                $stmt = $mysqli->prepare("INSERT INTO berita (judul, isi, foto) VALUES (?, ?, ?)");
...
if(isset($_POST['simpan'])) {
    if($modeaplikasi == "Demo") {
        ...
        if($_FILES['foto']['name'] == "") {
            $stmt = $mysqli->prepare("UPDATE berita SET judul = ?, isi = ? WHERE id = ?");
            ...
            $stmtv2 = $mysqli->prepare("SELECT * FROM berita WHERE id = ?");
                ...
                if(isset($foto) && unlink("$foto")) {
                    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
                        if($ukuran < 1044070){			
                            ...
                            $stmt = $mysqli->prepare("UPDATE berita SET judul = ?, isi = ?, foto = ? WHERE id = ?");
...
<html lang="en">
    <head>
        <title><?php echo $rowpengaturanweb['namaweb'];?> &mdash; Berita</title>
        ...
        <div id="app">
            <div class="main-wrapper main-wrapper-1">
                ...
                <div class="main-content">
                    ...
                    <?php
                    $stmtv321 = $mysqli->prepare("SELECT * FROM berita");
                    $stmtv321->execute();
